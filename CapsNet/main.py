...
def main():
    ...
    DATA_PATH = '../VAE/MNIST_DATA'
    BATCH_SIZE = 128
    train_loader = DataLoader(
        dataset=Subset(MNIST(root=DATA_PATH, download=True, train=True, transform=transform), list(range(1000))),
        batch_size=BATCH_SIZE,
        num_workers=4,
        shuffle=True)

    test_loader = DataLoader(
        dataset=Subset(MNIST(root=DATA_PATH, download=True, train=False, transform=transform), list(range(1000))),
        batch_size=BATCH_SIZE,
        num_workers=4,
        shuffle=True)
