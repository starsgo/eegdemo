...
def process_file(file_path, label):
    ...
    # 数据结构 data包含若干键值对，其中24个键名为wq_eegi(i=1...24)，对应的值是(62,33601)，对应24次测试，此时结果分4类，存在label里
    # data: { (N = 24)* "wq_eeg1": (C = 62, T = 33601)}
    # label: (24)
    ...
    for i in trils:
        ...
        data_time = data_time[:time//8*8,:,:]
        data_time = data_time.reshape(-1, 8, frequency, 62)
        # data_time: (T//f, f, C) 每个trail的T不一定相同
        label_temp = np.full(time//8, label[flag])
        # label_temp: (T//f)
        if label[flag] == 0:
            label0 += 1
            if label0 <= 4:
            ...
            if label3 <= 4:
                ...
                test_label.append(label_temp)
        flag += 1
    # 24次实验分4类，每类4组训练，2组测试
    print(f"labels: {label0} {label1} {label2} {label3}")
    # train_set: (16, T//f, f = 200, C = 62) 每个trail的T不一定相同
    # train_label: (16, T//f)
...
def process_files_in_directory(directory, label):
    ...
    print(f"all file train data: {np.array(train_data).shape}")
...
def load_SEEDIV():
    ...
    print(f"dataset size: "
          f"train_data_final:{np.array(train_data_final).shape} "
          f"train_label_final:{np.array(train_label_final).shape} "
          f"test_data_final:{np.array(test_data_final).shape} "
          f"test_label_final:{np.array(test_label_final).shape} ")
    return train_data_final, train_label_final, test_data_final, test_label_final


if __name__ == '__main__':
    result_dir = "/home/gxx/Documents/pythonProjects/datasets/dataset_SEED-IV/test/"
